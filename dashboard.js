...
async function showDashboard(ctx, user, edit = false) {
  ...
  const accs    = db.accounts.forUser(user.telegram_id);
  const accInfo = accs.length > 0 ? ` | 🔑 ${accs.length} account${accs.length > 1 ? 's' : ''}` : '';
  const settings = db.settings.get();
  const maxTrades = settings.max_active_trades ?? 5;

  const status = user.auto_trading ? '🤖 Auto Trading ON' : '⏹ Auto Trading OFF';

  const msg =
    `📊 <b>Dashboard</b>\n` +
    `👋 <b>${user.first_name || user.username || 'Trader'}</b>  |  ${subLabel}${accInfo}\n\n` +
    `💰 <b>Wallet</b>\n` +
    ...
    `🔓 Open Trades: <b>${openTrades.length} / ${maxTrades}</b>\n` +
    `Status: <b>${status}</b>\n` +
    `🔄 Synced: ${syncTime}`;
...
async function handleBalance(ctx) {
  ...
  if (!user.api_key && !user.active_account_id) {
  ...
  const { getUserClient } = require('./trading');
  ...
  try {
    ...
    const accs = db.accounts.forUser(user.telegram_id);
    const activeAcc = user.active_account_id ? db.accounts.findById(user.active_account_id) : null;
    const accLabel  = activeAcc ? ` — ${activeAcc.label}` : '';
    const msg =
      `💰 <b>Your Balance${accLabel}</b>\n\n` +
...
async function handleActiveTrades(ctx) {
  ...
  if (!open.length) {
    const msg = `📈 <b>Active Trades</b>\n\nNo open trades.\n<b>0 / ${maxTrades} slots used</b>\n\n<i>Binance positions are synced every 3 seconds.</i>`;
    const kb  = Markup.inlineKeyboard([
  ...
  let msg = `📈 <b>Active Trades (${open.length} / ${maxTrades})</b>\n\n`;
  const kbRows = [];
...
async function handleStatistics(ctx) {
  ...
  const msg =
    ...
    `━━━━━━━━━━━━━━━━━━━━━━\n` +
    `🔓 Open Trades: <b>${openCount} / ${settings.max_active_trades}</b>\n` +
    `🤖 Auto Trading: <b>${user.auto_trading ? 'ON ✅' : 'OFF ❌'}</b>`;
...
async function handleUserSettings(ctx) {
  ...
  const activeAcc = user.active_account_id ? db.accounts.findById(user.active_account_id) : null;
  const mkt       = activeAcc
    ? `${activeAcc.market_type === 'futures' ? '📊 Futures' : '📈 Spot'}${activeAcc.testnet ? ' [Testnet]' : ''} — ${activeAcc.label}`
    : (user.market_type
      ? `${user.market_type === 'futures' ? '📊 Futures' : '📈 Spot'}${user.testnet ? ' [Testnet]' : ''}`
      : 'Not set');

  const connected  = (user.api_key || user.active_account_id) ? '✅ Connected' : '❌ Not Connected';
  const syncTime   = user.last_binance_sync ? new Date(user.last_binance_sync).toLocaleTimeString() : 'Never';
  ...
  const msg =
    ...
    `📊 Market: ${mkt}\n` +
    `🔑 Saved Accounts: <b>${accs.length}</b>\n` +
...
// ─── SWITCH ACCOUNT ───────────────────────────────────────────────────────────
async function showSwitchAccount(ctx) {
  await ctx.answerCbQuery().catch(() => {});
  const user = db.users.findById(ctx.from.id);
  if (!user) return ctx.reply('Use /start first.');
  ...
  const accs = db.accounts.forUser(user.telegram_id);
  if (!accs.length) {
    const msg = '🔑 <b>No accounts saved yet.</b>\n\nAdd your first Binance API account below.';
    const kb  = Markup.inlineKeyboard([
      [Markup.button.callback('➕ Add Account', 'add_account')],
  ...
  let msg = `🔑 <b>Switch Account</b>\n\nSelect an account to use:\n\n`;
  const kbRows = [];
  for (const acc of accs) {
    const isActive = acc.id === user.active_account_id;
    const mktLabel = acc.market_type === 'futures' ? 'FUTURES' : 'SPOT';
    const tn       = acc.testnet ? ' [Testnet]' : '';
    msg += `${isActive ? '✅' : '⬜'} <b>${acc.label}</b>  (${mktLabel}${tn})\n`;
    kbRows.push([
      Markup.button.callback(
        `${isActive ? '✅ ' : ''}${acc.label} (${mktLabel})`,
        `set_account_${acc.id}`
      ),
      Markup.button.callback('🗑', `del_account_${acc.id}`),
    ]);
  }

  kbRows.push([
    Markup.button.callback('➕ Add New Account', 'add_account'),
    Markup.button.callback('🔙 Settings',       'user_settings'),
  ]);
...
async function handleAutoTradingOn(ctx) {
  ...
  if (!user.api_key && !user.active_account_id) return ctx.answerCbQuery('❌ Connect Binance API first.', { show_alert: true });
...
// ─── CLOSE TRADE ──────────────────────────────────────────────────────────────
async function handleCloseTradeAction(ctx, tradeId) {
